            ...
            for (const slideData of data.slides) {
              ...
              for (const shapeData of slideData.shapes) {
                ...
                if (type === 'text') {
                  ...
                  // const tableData = shapeData.rows.map((row: any) => {
                  //   return row.map((cellData: any) => {
                  //     const cellText = cellData.paragraphs.map((paragraphData: any) => {
                  //       const textRuns = paragraphData.textRuns.map((textRunData: any) => {
                  //         return {
                  //           text: textRunData.text,
                  //           options: {
                  //             fontFace: textRunData.font,
                  ...
                  //     return {
                  //       text: cellText.map((paragraph: any) => paragraph.text).join(''),
                  //       options: {
                  //         fillColor: cellData.fillColor ? cellData.fillColor.slice(1) : undefined,
                  //         valign: cellData.verticalAlignment,
                  //       },
                  //     };
                  //   });
                  // });

                  slide.addTable(shapeData.rows.map((row: any) => {
                    return row.map((cellData: any) => {
                      const { text } = cellData.paragraphs.reduce((acc: any, paragraphData: any) => {
                        paragraphData.textRuns.forEach((textRunData: any) => {
                          acc.text += textRunData.text;
                        });
                        acc.text += "\n";
